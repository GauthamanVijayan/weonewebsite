...
export class ConvexService {
  private client = inject(ConvexClient);
  ...
  async loadZones() {
    ...
    try {
      const zonesFromDB = await this.client.query(api.wards.getZones, {});
      
      // --- FIX FOR DUPLICATE ZONES ---
  ...
  async loadDistrictsByZone(zoneName: string) {
    ...
    this.clearDistricts();
    try {
      const districts = await this.client.query(api.wards.getDistrictsByZone, { zone: zoneName });
  ...
  async loadSubdistrictsByDistrict(districtName: string) {
    ...
    try {
      const subdistricts = await this.client.query(api.wards.getSubdistrictsByDistrict, { district: districtName });
  ...
  async loadWards(filters: { subdistrict: string; localBodyType: string; searchText: string; }) {
    this.wardsLoading.set(true);
    this.clearWards();
   try {
      // Call the new unified backend query
      const wards = await this.client.query(api.wards.getWards, filters);
